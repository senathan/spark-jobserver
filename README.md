...
----------
Here is the structured data model for your **Akamai onboarding and configuration system**, including sample table structures for **Properties**, **Hostnames**, **Contracts**, **Groups**, **Templates**, **Requests**, and more — all enriched with metadata.

---

### ✅ Table 1: `akamai_properties`

| Column          | Data Type | Key | Nullable | Description                    | Source | Source Field | Example     |
| --------------- | --------- | --- | -------- | ------------------------------ | ------ | ------------ | ----------- |
| `property_id`   | VARCHAR   | PK  | NO       | Unique Akamai Property ID      | PAPI   | propertyId   | prp\_12345  |
| `group_id`      | VARCHAR   | FK  | NO       | Group this property belongs to | PAPI   | groupId      | grp\_12345  |
| `property_name` | VARCHAR   |     | NO       | Name of the Akamai Property    | PAPI   | propertyName | example.com |
| `snow_ci`       | VARCHAR   |     | YES      | ServiceNow CI reference        | User   | snow\_ci     | CI123456    |
| `property_json` | JSON      |     | YES      | Raw property config (from API) | PAPI   | -            | `{...}`     |

---

### ✅ Table 2: `akamai_hostnames`

| Column             | Data Type | Key | Nullable | Description                      | Source     | Source Field | Example                                   |
| ------------------ | --------- | --- | -------- | -------------------------------- | ---------- | ------------ | ----------------------------------------- |
| `hostname`         | VARCHAR   | PK  | NO       | Unique hostname                  | PAPI       | cnameFrom    | [www.example.com](http://www.example.com) |
| `waf_status`       | VARCHAR   |     | YES      | WAF coverage status              | AppSec API | status       | Active                                    |
| `type_conditions`  | VARCHAR   |     | YES      | Hostname types, staging etc.     | User       | -            | Staging                                   |
| `active_change_id` | VARCHAR   |     | YES      | Change ID from latest deployment | PAPI       | -            | chg\_456                                  |
| `edge_hostname`    | VARCHAR   |     | YES      | Akamai edge hostname             | PAPI       | edgeHostname | edge.example.com                          |

---

### ✅ Table 3: `akamai_contracts`

| Column          | Data Type | Key | Nullable | Description          | Source | Source Field | Example     |
| --------------- | --------- | --- | -------- | -------------------- | ------ | ------------ | ----------- |
| `contract_id`   | VARCHAR   | PK  | NO       | Contract ID          | PAPI   | contractId   | ctr\_1-ABCD |
| `contract_name` | VARCHAR   |     | NO       | Akamai contract name | PAPI   | contractName | Global CDN  |

---

### ✅ Table 4: `akamai_groups`

| Column            | Data Type | Key | Nullable | Description            | Source | Source Field | Example  |
| ----------------- | --------- | --- | -------- | ---------------------- | ------ | ------------ | -------- |
| `group_id`        | VARCHAR   | PK  | NO       | Group ID               | PAPI   | groupId      | grp\_123 |
| `group_name`      | VARCHAR   |     | NO       | Group name             | PAPI   | groupName    | DevOps   |
| `parent_group_id` | VARCHAR   | FK  | YES      | Parent group reference | User   | -            | pg\_101  |

---

### ✅ Table 5: `akamai_parent_group`

| Column                | Data Type | Key | Nullable | Description            | Source | Source Field | Example      |
| --------------------- | --------- | --- | -------- | ---------------------- | ------ | ------------ | ------------ |
| `included_group_id`   | VARCHAR   | PK  | NO       | Group ID included      | Manual | -            | grp\_321     |
| `included_group_name` | VARCHAR   |     | NO       | Name of included group | Manual | -            | SecurityTeam |

---

### ✅ Table 6: `akamai_templates`

| Column          | Data Type | Key | Nullable | Description          | Source | Source Field | Example                |
| --------------- | --------- | --- | -------- | -------------------- | ------ | ------------ | ---------------------- |
| `template_id`   | INT       | PK  | NO       | Template ID          | User   | -            | 1                      |
| `template_name` | VARCHAR   |     | NO       | Template description | User   | -            | Akamai Standard Config |

---

### ✅ Table 7: `akamai_onboarding_requests`

| Column                   | Data Type | Key | Nullable | Description                  | Source | Source Field | Example                                       |
| ------------------------ | --------- | --- | -------- | ---------------------------- | ------ | ------------ | --------------------------------------------- |
| `onboard_request_id`     | UUID      | PK  | NO       | Unique onboarding request ID | System | -            | uuid-abc-123                                  |
| `template_id`            | INT       | FK  | NO       | Template selected            | User   | -            | 1                                             |
| `contract_id`            | VARCHAR   |     | NO       | Contract ID used             | User   | -            | ctr\_1-ABCD                                   |
| `group_id`               | VARCHAR   |     | NO       | Group ID selected            | User   | -            | grp\_456                                      |
| `security_policy`        | VARCHAR   |     | NO       | Security policy applied      | User   | -            | policy-xyz                                    |
| `security_configuration` | VARCHAR   |     | NO       | Websec config info           | User   | -            | config-123                                    |
| `notes`                  | TEXT      |     | YES      | Optional justification       | User   | -            | External exposure                             |
| `who_updated`            | VARCHAR   |     | NO       | Last updated by              | System | -            | [admin@example.com](mailto:admin@example.com) |
| `created_at`             | TIMESTAMP |     | NO       | Timestamp of request         | System | -            | 2025-06-22 10:00                              |

---

*(Remaining tables will continue in next message for clarity...)*


Continuing with the remaining key tables from your Akamai onboarding system:

---

### ✅ Table 8: `akamai_onboarding_hostnames`

| Column                    | Data Type | Key | Nullable | Description                            | Source         | Source Field | Example            |
| ------------------------- | --------- | --- | -------- | -------------------------------------- | -------------- | ------------ | ------------------ |
| `hostname_id`             | UUID      | PK  | NO       | Unique ID for each hostname onboarded  | System         | -            | uuid-5678          |
| `onboard_request_id`      | UUID      | FK  | NO       | Foreign key to onboarding request      | System         | -            | uuid-abc-123       |
| `hostname`                | VARCHAR   |     | NO       | Hostname being onboarded               | User Input     | -            | app.example.com    |
| `itsi_code`               | VARCHAR   |     | YES      | Associated ITSI code                   | User Input     | -            | ITSI001            |
| `internet_facing`         | BOOLEAN   |     | NO       | Is the hostname internet facing?       | User Input     | -            | TRUE               |
| `environment`             | VARCHAR   |     | NO       | UAT, PROD, etc.                        | User Input     | -            | UAT                |
| `gtm`                     | VARCHAR   |     | YES      | GTM configuration                      | User Input     | -            | gtm.example.com    |
| `dns_record`              | VARCHAR   |     | YES      | Resolved DNS value                     | Cloudflare DNS | -            | 192.168.1.1        |
| `certificate_status`      | VARCHAR   |     | YES      | Cert lifecycle status                  | CPS API        | -            | Submitted          |
| `origin_fqdn`             | VARCHAR   |     | NO       | FQDN of the origin server              | User Input     | -            | origin.example.com |
| `certificate_domain_type` | VARCHAR   |     | YES      | Primary or SAN domain                  | CPS API        | -            | Primary            |
| `certificate_name`        | VARCHAR   |     | YES      | CN used in CSR                         | CPS API        | -            | example.com        |
| `max_mbps`                | INT       |     | YES      | Max expected bandwidth                 | User Input     | -            | 100                |
| `resilience`              | VARCHAR   |     | YES      | Failover type (Standard/Enhanced)      | User Input     | -            | Standard           |
| `x_forwarded_for`         | BOOLEAN   |     | YES      | X-Forwarded-For header enabled         | User Input     | -            | TRUE               |
| `hsts`                    | BOOLEAN   |     | YES      | HTTP Strict Transport Security enabled | User Input     | -            | TRUE               |
| `tls_version`             | VARCHAR   |     | YES      | TLS protocol version supported         | Diagnostic API | -            | TLS 1.2            |
| `failaway_type`           | VARCHAR   |     | YES      | Failaway config type                   | PAPI           | -            | Standard           |
| `failaway_outage`         | VARCHAR   |     | YES      | Custom outage behavior                 | PAPI           | -            | Custom Page        |
| `php_resource_available`  | BOOLEAN   |     | YES      | Is PHP resource required?              | User Input     | -            | FALSE              |

---

These tables together form a **complete schema** for your Akamai onboarding automation workflow. You can now:

* Track requests from Excel to onboarding.
* Handle retries, updates, and validation.
* Enrich hostnames with security and infrastructure attributes.
* Query status from PAPI, CPS, AppSec APIs and Cloudflare DNS.
* Extend with audit/history or processing queues if needed.

---

Would you like me to export these table definitions as:

* CSV files?
* SQL `CREATE TABLE` scripts?
* A data dictionary Excel file?

Let me know your preferred format!

-----------Thanks for the reminder! Below are **4-line summary descriptions** for each table, explaining the context, data source, and inclusion logic:

---

### 🟩 Table 1: `akamai_properties`

This table contains Akamai property configurations fetched via PAPI.
Each property is linked to a group and includes metadata like name and raw JSON.
It represents the central entity tied to edge delivery, WAF, and DNS settings.
Only properties visible via the Akamai API for your credentials are included.

---

### 🟩 Table 2: `akamai_hostnames`

This table stores hostnames mapped to properties, configs, and edge settings.
WAF status, edge hostname, and change tracking are included.
Hostnames are pulled from `/properties/{id}/hostnames` and enriched via AppSec API.
Only active or onboarded hostnames are persisted here.

---

### 🟩 Table 3: `akamai_contracts`

This holds Akamai contract metadata, used to scope available properties and features.
Each contract defines usage boundaries such as product types and regions.
Contracts are fetched from `/contracts` endpoint.
All visible contracts under your client account are stored.

---

### 🟩 Table 4: `akamai_groups`

Represents Akamai access control groups that organize properties and configs.
Groups help define permission boundaries and team ownership.
Data is fetched from `/groups` API.
Only groups tied to onboarded properties or parent group inclusion are stored.

---

### 🟩 Table 5: `akamai_parent_group`

Stores only groups that are explicitly marked as *included* for onboarding.
This acts as a whitelist or mapping layer over `akamai_groups`.
Manually maintained based on governance policies.
Used to filter which groups are selectable for new onboarding requests.

---

### 🟩 Table 6: `akamai_templates`

Templates define standardized onboarding blueprints (TLS, headers, policies).
Used to pre-fill onboarding forms and ensure consistent configuration.
Maintained manually or via internal tooling (not fetched from Akamai).
Each onboarding request links to one template for reproducibility.

---

### 🟩 Table 7: `akamai_onboarding_requests`

Tracks user-initiated onboarding events, linked to templates, groups, and contracts.
Stores metadata like requester info, security config, and notes.
System-generated UUID acts as primary identifier.
Each request can onboard multiple hostnames (via child table).

---

### 🟩 Table 8: `akamai_onboarding_hostnames`

Captures hostname-level details tied to onboarding requests.
Includes internet exposure, TLS expectations, failover, and DNS data.
Hostnames are later mapped to property configs and certificates.
Each row represents one unique FQDN with all onboarding metadata.

---

### 🟩 Table 9: `akamai_host_information`

This table stores technical attributes and behaviors of hostnames.
Data includes HTTP method rules, DNS TTLs, and failover specifics.
Pulled from diagnostic tools or AppSec match targets as needed.
Useful for audit, troubleshooting, and validation purposes.

---

### 🟩 Table 10: `akamai_certificate_information`

Stores certificate lifecycle info fetched from CPS API (`/enrollments`).
Mapped against onboarded hostnames to track status and validity.
Classifies domains as primary or secondary (based on CSR match).
Not dependent on Venafi, but compatible for post-CSR upload tracking.

---

### 🟩 Table 11: `akamai_property_config`

Maps property hostnames to config IDs and version numbers (PAPI + AppSec).
Helps correlate hostnames with their security policies and coverage.
Versioning ensures accuracy even during rollouts or change windows.
Critical for linking security posture to deployment state.

---

### 🟩 Table 12: `akamai_security_policy`

Stores security policy metadata linked to configurations.
Coverage status is determined using hostname-match target APIs.
Policies include bot, WAF, rate limiting, and other AppSec rules.
Each policy links back to one config but can cover multiple hostnames.

---

Let me know if you’d like this formatted in Excel, Confluence Markdown, or JSON/YAML for automated schema registration.
